...
export class MedicoService {
  ...
  getMedicoById(uid: string){
    return this._http
                .get(`${base_url}/medicos/${uid}`,this.headers)
                .pipe(
                  map((resp: any) => {
                    let { medico, ...resto } = resp;
                    const { nombre, id, createdByUser, hospital , img } = medico;
                    // createdByUser medico
                    const nombre_UM = createdByUser.nombre;
                    const email_UM = createdByUser.email;
                    const role_UM = createdByUser.role;
                    ...
                    medico = new Medico(nombre, img, userMed , hospiMed , id);
